...
// لینک فونت Vazir رو داخل index.html اضافه کن:
...
export function Header() {
  const { model, setModel, darkMode, setDarkMode, clearChat } = useChat();

  return (
    <AppBar position="sticky" color="primary" enableColorOnDark>
      <Toolbar sx={{ justifyContent: "space-between", flexWrap: "wrap" }}>
        <Typography variant="h6" sx={{ flexGrow: 1 }}>
          دستیار هوش مصنوعی پویان
        </Typography>
...
export function Messages() {
  const { messages, darkMode, displayedAnswer, typing } = useChat();

  // آرایه رفرنس‌ها برای پیام‌ها
  ...
  useEffect(() => {
    ...
    if (lastAssistantRef) {
      // اسکرول به ابتدای پیام دستیار
      lastAssistantRef.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  }, [messages]);

  // برای پیام در حال تایپ، رفرنش جداست
      ...
      {messages.map(({ id, role, content }, index) => (
        <Paper
          ...
          sx={{
            ...
            bgcolor: role === "user" ? "primary.main" : darkMode ? "grey.700" : "grey.300",
            color: role === "user" ? "primary.contrastText" : darkMode ? "grey.100" : "text.primary",
            borderRadius: 3,
            boxShadow: role === "user" ? "0 3px 10px rgba(0, 123, 255, 0.4)" : "none",
            whiteSpace: "pre-wrap",
...
export function InputArea() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.answer) {
        // نمایش کلمه به کلمه جواب
        const words = data.answer.split(/(\s+)/); // حفظ فاصله‌ها
        let index = 0;
        setDisplayedAnswer("");
        setTyping(true);
  ...
  return (
    <Box
      ...
      sx={{ display: "flex", p: 2, bgcolor: "background.paper", borderTop: 1, borderColor: "divider" }}
    >
      <TextField
        multiline
        minRows={2}
      ...
      <Button
        ...
        sx={{ ml: 2, borderRadius: 3, minWidth: 100 }}
